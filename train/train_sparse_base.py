...
agent = DDPG(obs_shape, action_shape, action_min=action_min, action_max=action_max,
             noise_decrease=1 / (num_epoch * episode_num * trajectory_len), polyak=0.05, critic_lr=1e-3, actor_lr=1e-3, noise_scaler=0.8, task_name='DDPG_base')
...
for epoch in range(num_epoch):
    ...
    for episode in range(episode_num):
        ...
        for t in range(trajectory_len):
            ...
            reward = 1 if distance < 0.05 else 0
...
np.save('base_DDPG_2', success_rate_array)
writer.close()
